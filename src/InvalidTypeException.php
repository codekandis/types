...
class InvalidTypeException extends RuntimeException implements InvalidTypeExceptionInterface
{
	 ...
	 * @var string
	 */
	public const string EXCEPTION_MESSAGE_INVALID_TYPE = 'The type `%s` is invalid.';

	/**
	 * Represents the exception message if a type is invalid and expected types are provided.
	 * @var string
	 */
	public const string EXCEPTION_MESSAGE_INVALID_TYPE_AND_EXPECTED_TYPES = 'The type `%s` is invalid. `%s` expected.';
	...
	{
		return new static(
			sprintf( static::EXCEPTION_MESSAGE_INVALID_TYPE, $invalidType )
		);
	}
	...
	public static function with_invalidTypeAndExpectedTypes( string $invalidType, string ...$expectedTypes ): static
	{
		return new static(
			sprintf(
				static::EXCEPTION_MESSAGE_INVALID_TYPE_AND_EXPECTED_TYPES,
				$invalidType,
