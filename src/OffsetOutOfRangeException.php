...
class OffsetOutOfRangeException extends InvalidOffsetException implements OffsetOutOfRangeExceptionInterface
{
	 ...
	 * @var string
	 */
	public const string EXCEPTION_MESSAGE_WITH_OUT_OF_RANGE_OFFSET = 'The offset `%s` is out of range.';

	/**
	 * Represents the exception message with the out of range offset, an expected minimum offset and an expected maximum offset.
	 * @var string
	 */
	public const string EXCEPTION_MESSAGE_WITH_OUT_OF_RANGE_OFFSET_EXPECTED_MIN_OFFSET_AND_EXPECTED_MAX_OFFSET = 'The offset `%s` is out of range. `%s <= offset <= %s` expected.';
